...
public class VideoStreamCapture extends UseCase {
    ...
    private androidx.camera.core.EncodedBufferHandler mEncodedBufferHandler = null;
 ...
 /*       try {
        ...
        mAudioHandler.post(
                new Runnable() {
                    @Override
                    public void run() {
                        VideoStreamCapture.this.audioEncode(postListener);
                    }
                });
        ...
        mVideoHandler.post(
                new Runnable() {
                    @Override
                    public void run() {
                        Log.d(TAG, "started video handler");
                        boolean errorOccurred = VideoStreamCapture.this.videoEncode(postListener,
                                cameraId, resolution);
                        if (!errorOccurred) {
//                            postListener.onVideoSaved(saveLocation);
    ...
    private boolean writeVideoEncodedBuffer(int bufferIndex) {
        ...
        byte[] outBytes = new byte[outputBuffer.remaining()];
...
//        if (mVideoBufferInfo.size > 0) {
            ...
            synchronized (mMuxerLock) {
                ...
                if (mEncodedBufferHandler != null) {
                    mEncodedBufferHandler.videoBufferReady(outBytes,
        ...
        if (mAudioBufferInfo.size > 0
            ...
            try {
                synchronized (mMuxerLock) {
                    ...
                    buffer.get(outBytes);
                    if (mEncodedBufferHandler != null) {
                        mEncodedBufferHandler.audioBufferReady(outBytes,
    ...
    boolean videoEncode(@NonNull OnVideoSavedCallback videoSavedCallback, @NonNull String cameraId,
        ...
        if (mEncodedBufferHandler != null) {
            mEncodedBufferHandler.recordingStopped();
        }
...
//                                }
    ...
    public void setEncodedBufferHandler(androidx.camera.core.EncodedBufferHandler encodedBufferHandler) {
        mEncodedBufferHandler = encodedBufferHandler;
    }
    ...
    public static final class Defaults
            implements ConfigProvider<VideoStreamCaptureConfig> {
